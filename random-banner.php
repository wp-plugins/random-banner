...
echo '<!-- Buffercode.com Random Banner Plugin --><div class="random_banner_css"> <a href="'.$buffercode_RBanner_img_url_WO_empty[$buffercode_RBanner_rand_numb].'" alt="'.$buffercode_RBanner_img_url_WO_empty[$buffercode_RBanner_rand_numb].'" target="_blank">';
echo '<img src="'.$buffercode_RBanner_img_var_WO_empty[$buffercode_RBanner_rand_numb].'"/>';
echo '</a></div> <!-- Buffercode.com Random Banner Plugin -->'; 
/* Buffercode.com wordpress Banner upload plugin */
}
echo $after_widget;
}
...
global $my_settings_page;
	$my_settings_page = add_options_page( 'Random Banner', 'Random Banner', 'manage_options', __FILE__, 'buffercode_RBanner_menu_setting' );
	...
	<tr valign="top"><td></td>
       ...
       <tr valign="top">
		...
		<td></td>
	       ...
	       <tr valign="top">
		   <td></td>
                   <td><?php submit_button();  ?><br />For Suggestion - <a href="http://buffercode.com/">Buffercode.com</a> <br /> If you find this plugin valuable ? kindly,<br /> <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=7DHAEMST475BY"><img src="<?php echo plugins_url('images/donate.png', __FILE__); ?>" alt="Donate" /></a></td>
		   <td></td>
        </tr>
    </table>
...
function buffercode_RBanner_uploader_scripts($hook) {
  global $my_settings_page;
if ( $my_settings_page != $hook )
    return;
    wp_enqueue_script('media-upload');
    wp_enqueue_script('thickbox');
    wp_enqueue_script( 'random-banner-script', plugin_dir_url( __FILE__ ) . '/js/upload-js.js' );

}
add_action('admin_enqueue_scripts', 'buffercode_RBanner_uploader_scripts');
add_action('admin_enqueue_scripts', 'buffercode_RBanner_uploader_styles'); 
function buffercode_RBanner_uploader_styles() {
   wp_enqueue_style('thickbox');
   
}
// CSS for webpage.
function buffercode_Rbanner_uploader_page_style(){
    wp_enqueue_style( 'rbanner-css',  plugin_dir_url( __FILE__ ).'/css/rbanner.css'); 
}
add_action( 'wp_enqueue_scripts', 'buffercode_Rbanner_uploader_page_style' );
